...
description: >
  ...
  Use this skill when the user mentions: "business plan", "business planning", "business analysis",
  "market analysis", "revenue model", "monetization plan", "go-to-market", "GTM strategy", "MVP plan",
  "product roadmap", "startup plan", "app business plan", or describes a product idea and wants to
  understand its commercial viability. Also triggers on: "is this a good idea?", "how do I make money
  from this?", "help me plan my app/product/service".
...
If the conversation already contains a detailed idea, extract answers directly — do not ask again.
...
If web search is unavailable, use conservative estimates and label them clearly.

---

## Step 3 — Write the Business Plan

Follow the structure in `template.md` exactly.
Fill every section — never leave a section as "TBD" or blank.

**Section order:**
1. Executive Summary (5–7 bullet KPIs)
2. Market Opportunity (use `market-sizing.md` for TAM/SAM/SOM methodology)
3. Product Definition & Feature Set
4. Tech Stack & Architecture
5. MVP Roadmap (day-by-day or week-by-week)
6. Revenue Model with projections table (use `revenue-models.md` for patterns)
7. Cost Structure
...
- **Solo-developer friendly:** If team = 1 person, emphasize no-server/serverless options, free tiers, and automation to reduce support burden.

---

## Reference Files

- `template.md` — Full section-by-section template with example tables
- `revenue-models.md` — Revenue model patterns (Freemium, SaaS, Marketplace, etc.) with benchmark conversion rates
- `market-sizing.md` — How to estimate TAM/SAM/SOM for common product categories
