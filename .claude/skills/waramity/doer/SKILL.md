...
1. List both `.waramity/requirement/` and `.waramity/wip/` to see all REQ files (filenames show status emoji: 🟡 Planned, 🚧 WIP, ⏳ Wait to Review, ✅ Done)
...
3. Read the matched individual file (e.g. `.waramity/requirement/⏳ 0005-auto-smooth-paint-mask-edges.md` or `.waramity/wip/🚧 0003-fix-some-bug.md`)
...
**NEVER delete or remove any file under `.waramity/requirement/` or `.waramity/wip/`. Only rename or edit in-place.**
...
3. Rename the file to reflect new status:
   - If status is now `⏳ Wait to Review`:
     ```bash
     OLD_FILE=$(ls .waramity/requirement/*${REQ_NUMBER}-*.md 2>/dev/null | head -1)
     mv "$OLD_FILE" ".waramity/requirement/⏳ ${REQ_NUMBER}-${SLUG}.md"
     ```
   - If status is `🚧 WIP` (user requests mid-implementation):
     ...
     mv "$OLD_FILE" ".waramity/requirement/🚧 ${REQ_NUMBER}-${SLUG}.md"
     ```
4. **Verify the rename succeeded** — run `ls .waramity/requirement/` and confirm the file now has the emoji prefix. If the rename failed, retry it before proceeding.
5. **Leave WIP files alone** — do NOT delete or move files from `.waramity/wip/`. They stay as-is.
...
- **WIP file exists for the REQ**: Leave it untouched — never delete or move files from `.waramity/wip/`
